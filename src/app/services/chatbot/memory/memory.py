...
class MemorySystem:
    ...
    async def _extract_entities(self, content: str) -> Set[str]:
        ...
        try:
            ...
            try:
                # Check if result content is empty or None
                if not result.content or not result.content.strip():
                    logger.warning("Empty entity extraction result from LLM")
                    return set()
                ...
                try:
                    ...
                    if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
                        ...
                        logger.warning(f"No valid JSON found in LLM response: {result.content[:200]}...")
                        return set()
                ...
                logger.error(f"LLM response was: {result.content[:500] if result.content else 'None'}")
                return set()
                return set()
    ...
    async def _extract_topics(self, content: str) -> Set[str]:
        ...
        try:
            ...
            try:
                # Check if result content is empty or None
                if not result.content or not result.content.strip():
                    logger.warning("Empty topic extraction result from LLM")
                    return set()
                ...
                try:
                    ...
                    if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
                        ...
                        logger.warning(f"No valid JSON found in topic extraction LLM response: {result.content[:200]}...")
                        return set()
